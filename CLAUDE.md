...
- **System of record, if/when built**: Postgres is canonical (events, tasks, projects, decisions, facts, provenance). Notion is a human-readable mirror, never the source of truth. Every derived fact/event/decision traces back to a `SourceRecord` with `source_type`, `source_id`, `retrieved_at`. Until then, Notion is the de facto Milestone 0 system of record for routine-persisted domain data (not yet a "mirror" of anything) — see `docs/memory-architecture.md` for how that and Claude Code's own session memory map onto this future state.
