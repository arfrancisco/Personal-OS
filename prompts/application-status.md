...
2. Search Gmail in two passes, since outcomes don't always land within a few days of applying:
   - **Recent-activity pass**: last ~3-5 days, or since the tracker's most recent "Last Update" if longer — for new application confirmations, interview invites/reminders, and assessment nudges.
   - **Resolution sweep**: a wider look-back (~14 days) specifically for rejection/decision/offer language — e.g. "unfortunately", "regret", "moving forward with other candidates", "not selected", "no longer under consideration", "unsuccessful", "position has been filled", "declined", "decided not to proceed", as well as offer language. Outcomes can arrive well after the original application date and after the recent-activity window has closed, so don't skip this pass even if the recent-activity pass came back quiet. Focus this sweep on rows currently "Awaiting Response," "Interviewing," or "Action Needed" — rows already "Rejected," "Offer," or "Withdrawn" are closed and don't need re-checking.
   - For any thread either pass turns up, don't conclude from the preview snippet alone — fetch the full thread body before deciding what it says. Snippets truncate mid-sentence and can cut off right before the decisive line (e.g. showing only the polite opening of a rejection email), which will misclassify a closed application as still open.
   - Search Calendar for scheduled interviews going forward from today.
3. Reconcile: for each new or changed item found in Gmail/Calendar, update the matching Notion row (Status, Last Update, Next Action, Notes) or create a new row if it's a new application. Match on Company + Role, not just Company. If a company/role in the tracker shows no new email activity after both passes, leave it as-is — don't assume it's still accurate, but don't overwrite it either.
